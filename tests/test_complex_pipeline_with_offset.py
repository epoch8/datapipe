...
from datapipe.types import IndexDF, Required, JoinSpec
...
def test_complex_pipeline_with_offset_optimization(dbconn):
    ...
    pipeline = Pipeline(
        [
            BatchTransform(
                ...
                inputs=[
                    Required("item", join_keys={"item_id": "item_id"}),  # inner join с reverse join
                    Required("pipeline", join_keys={"pipeline_id": "pipeline_id"}),  # inner join с reverse join
                    "prediction",  # full join, основная таблица (содержит все transform_keys)
                    Required("keypoint"),  # inner join, референсная таблица без reverse join
                ],
                outputs=["output"],
...
def test_complex_train_pipeline_with_offset_optimization(dbconn):
    ds = DataStore(dbconn, create_meta_table=True)
    catalog = Catalog(
        {
    ...
    pipeline = Pipeline(
        [
            BatchTransform(
                func=train,
                inputs=[
                    "frozen_dataset",  # full join (часть transform_keys)
                    "train_config",  # full join (часть transform_keys)
                    "pipeline",  # full join, выходная таблица
                    "pipeline__is_trained_on__frozen_dataset",  # full join, выходная таблица
                ],
    ...
    pipeline = Pipeline(
        [
            ...
            BatchTransform(
                ...
                inputs=[
                    Required("tbl_image", join_keys={"image_id": "image_id"}),  # inner join с reverse join
                    Required("tbl_image__attribute", join_keys={"image_id": "image_id"}),  # inner join с reverse join
                    "tbl_prediction",  # full join, содержит все transform_keys (image_id, model_id)
                    Required("tbl_best_model", join_keys={"model_id": "model_id"}),  # inner join с reverse join
                ],
                outputs=["tbl_output"],
...
def test_complex_transform_with_many_recordings_N100_with_offset_optimization(dbconn):
    complex_transform_with_many_recordings(dbconn, N=100)


def test_complex_transform_with_many_recordings_N1000_with_offset_optimization(dbconn):
    complex_transform_with_many_recordings(dbconn, N=1000)


@pytest.mark.skip(reason="fails on sqlite")
def test_complex_transform_with_many_recordings_N10000_with_offset_optimization(dbconn):
