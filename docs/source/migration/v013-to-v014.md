...
## DatatableTransform can become BatchTransform
...
BatchTransform(
    ...
    # Important: empty transform_keys so the step operates on the whole input at once
    transform_keys=[],
)
```

## Migrate transform meta tables

After upgrading, seed per-step transform meta from existing output meta when the new `{step}_meta` table is empty:

```bash
datapipe table migrate-transform-tables
# optional filters (same semantics as `datapipe step`):
datapipe table migrate-transform-tables --name=my_step
datapipe table migrate-transform-tables --labels=stage=etl
```

Skips steps whose transform meta already has rows. See [CLI `table migrate-transform-tables`](../reference/cli.md).
