...
class OpsSpecRegistry:
    ...
    def _validate_spec(self, spec: DatapipeOpsSpec, catalog: Catalog | None, db: DataStore | None) -> None:
        ...
        if (
            ...
            from datapipe_app_ml_ops.ops.ops_spec_validation import validate_ml_spec_extensions

            validate_ml_spec_extensions(spec, catalog, db, registry=self)
    ...
    def _table_ids(self, spec: DatapipeOpsSpec) -> set[str]:
        from datapipe_app_ml_ops.ops.ops_spec_validation import collect_ml_table_ids

        tables = set(spec.data.tables if spec.data else [])
        tables.update(collect_ml_table_ids(spec))
