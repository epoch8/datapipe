describe("mlOpsApi filter query serialization", () => {
    it("serializes filters as JSON query param", async () => {
        ...
        const { mlOpsApi } = await import("@datapipe/ui-ml/api/mlOps");
        await mlOpsApi.getOpsMetricRows("pipeline-1", "spec-1", "table-1", {
            filter_mode: "or",
            filters: [
