...
class FakeStep(ComputeStep):
    ...
    def run_full(self, ds, run_config=None, executor=None):
        ...
        if self.report_progress and callback is not None:
            callback.on_step_progress(self, 0, 2)
            callback.on_step_progress(self, 1, 2)
            callback.on_step_progress(self, 2, 2)
        if self.fail:
            raise RuntimeError(f"{self.name} failed")


def _run_config_with_callbacks(*callbacks: RunCallback) -> RunConfig:
    return RunConfig(callback=CompositeRunCallback(list(callbacks)))
