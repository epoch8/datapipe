...
class MetaPlane:
    ...
    def create_chain_transform_meta(
        ...
        """
        Create a `ChainTransformMeta` describing metadata for a chain transform with the
        specified input, previous and output tables.

        transform_keys/rank control processing order; the concrete
        implementation should rely on TableMeta objects from the same MetaPlane.
...
class ChainTransformMeta:
    ...
    def compute_transform_schema(
        ...
        for cp in previous:
            all_keys.update({col.name: col for col in cp.primary_schema})

        if transform_keys is not None:
            return (transform_keys, [all_keys[k] for k in transform_keys])

        assert len(inputs) > 0 and len(previous) > 0, "At least one input and previous table is required to infer transform keys"

        inp_p_keys = set.intersection(*[set(inp.primary_keys) for inp in inputs])
        prv_p_keys = set.intersection(*[set(prv.primary_keys) for prv in previous])

        inp_prv_p_keys = set.intersection(inp_p_keys, prv_p_keys)

        assert len(inp_prv_p_keys) > 0, "No intersection for primary keys in input and previus tables"

        if len(outputs) == 0:
            return (list(inp_prv_p_keys), [all_keys[k] for k in inp_prv_p_keys])

        out_p_keys = set.intersection(*[set(out.primary_keys) for out in outputs])
        
        assert len(out_p_keys) > 0

        inp_prv_out_p_keys = set.intersection(inp_prv_p_keys, out_p_keys)
        assert len(inp_prv_out_p_keys) > 0

        return (list(inp_prv_out_p_keys), [all_keys[k] for k in inp_prv_out_p_keys])
    ...
    def get_new_process_idx_count(
        ...
        """
        Count how many indices will add to transform  index (honoring `run_config`).
        """
