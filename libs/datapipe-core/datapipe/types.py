...
def get_pipeline_output_table(output: PipelineOutput) -> TableOrName:
...
def get_pairwise_primary_intersections_in_tables(
    ...
    pairwise_primary_intersections_in_tables = {(table_names[t[0]], table_names[t[1]]): nt(*t) for t in pairs}
...
def get_all_equivalence_tables(schemas: list[DataSchema], table_names: list[str]) -> list[EquivalenceTables]:
    ...
    for table_name in table_names:
        ...
        if non_empty_sets_intersection is None:
            ...
            all_equalience_tables.append(EquivalenceTables(table_names=[table_name], idxs=sorted(idxs)))
