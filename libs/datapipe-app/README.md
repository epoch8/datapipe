...
ClickHouse run-log tables are also **off by default** on API startup
(`RunLogsBackend.clickhouse(..., create_table=False)`); `datapipe db create-all` creates them
when a ClickHouse `run_logs_backend` is configured (or pass `create_table=True` / call
`store.ensure_table()`).
