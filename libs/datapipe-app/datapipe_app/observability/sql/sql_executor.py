...
def execute_readonly_query(
    ...
    *,
    ...
    """Execute a validated user-authored SELECT.

    ``text()`` is intentional here: the analytics SQL endpoint accepts arbitrary
    read-only queries that cannot be represented as fixed SQLAlchemy expressions.
    """
    _validate_sql(sql)
    rewritten = _rewrite_sql(
        sql,
        qualified_table_map=qualified_table_map,
        bare_table_map=bare_table_map,
    )
