...
@dataclass
class RecordingRunCallback(RunCallback):
    """Per-run run callback with a stable ``run_id``.
    ...
    def on_run_start(self, steps: Sequence[ComputeStep]) -> None:
        if self.cli_announce:
            print(f"Recording run {self.run_id} for pipeline '{self.recorder.pipeline_id}' (view in Ops UI → Runs)")
...
class RunRecorder:
    ...
    or background runs::
        ...
        run_steps(ds, steps, executor=executor, run_config=RunConfig(callback=cb))
        ...
        *,
        ...
        Typical use::
            ...
            with recorder.run_scope(trigger="api") as cb:
                run_steps(ds, steps, run_config=RunConfig(callback=cb))
        """
