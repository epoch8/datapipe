...
import pandas as pd
from datapipe.compute import Catalog, ComputeStep, Pipeline, PipelineStep
...
@dataclass
class _TransformStateProgressCallback(RunCallback):
    transform_state: models.RunStepResponse
    chunk_size: int

    def on_step_progress(self, step: ComputeStep, completed: int, total: int | None) -> None:
        self.transform_state.total = total if total is not None else 0
        self.transform_state.status = "running"
        self.transform_state.processed = completed * self.chunk_size
...
def run_step(
    ...
    def _run() -> None:
        ...
        step.run_full(
            ds=ds,
            run_config=RunConfig(callback=_TransformStateProgressCallback(transform_state, step.chunk_size)),
...
def register_pipeline_ui_routes(
    ...
    def get_graph(stage: Optional[str] = Query(None)) -> models.GraphResponse:
        selected_steps = filter_steps_by_labels(steps, labels=[("stage", stage)]) if stage else steps
        ...
        try:
            while True:
                ...
                if not isinstance(step, BaseBatchTransformStep):
                    ...
                    run_steps_task.add_done_callback(lambda _: running_steps_helper.set_job_as_finished(transform))
