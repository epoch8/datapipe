    ...
    for stage in stage_order:
        ...
        result.append(
            models.StageItem(
                stage=stage,
                status=state,
                steps=[models.StageStepStatus(**s) for s in statuses],
            )
        )
    return result


def build_stage_edges(steps: list[ComputeStep]) -> list[models.StageEdge]:
...
def discover_pipeline_stages(
    ...
    for pipeline_step in pipeline.steps:
        for k, v in pipeline_step_labels(pipeline_step):
            if k == "stage" and v not in seen:
                stages.append(v)
                seen.add(v)
    return stages
...
# Keep Catalog imported for type parity with upstream helpers that accept it.
_ = Catalog
