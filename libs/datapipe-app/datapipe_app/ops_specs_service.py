...
class OpsSpecsService:
    ...
    def frozen_filter_columns(self, spec_id: str) -> list[OpsColumn]:
        ...
        if spec.frozen_dataset is None:
            return []
        return list(spec.frozen_dataset.columns)
    ...
    def _spec_summary(self, spec: DatapipeOpsSpec) -> dict[str, Any]:
        return {"id": spec.id, "title": spec.title, "description": spec.description, "icon": spec.icon, "color": spec.color, "has_frozen_datasets": spec.frozen_dataset is not None, "has_training": spec.training is not None, "has_image": spec.data is not None and spec.data.image_view is not None, "has_model_predictions": spec.model is not None and spec.model.prediction_view is not None, "metric_tables_count": len(spec.metrics), "class_metric_tables_count": len(spec.class_metrics), "tags": list(spec.tags)}
