...
class OpsSpecsService:
    ...
    def list_specs(self, pipeline_id: str) -> dict[str, Any]:
        return {
            ...
            "specs": [self._spec_summary(spec) for spec in self.registry.list()],
        }
    ...
    def frozen_overview(self) -> dict[str, Any]:
        ...
        for spec in specs:
            ...
            cards.append(
                self._overview_card(spec)
                | {
                    "status": "empty" if count == 0 else "healthy",
                    "snapshots_count": count,
                    "latest_snapshot": recent_rows[0] if recent_rows else None,
                    "split": recent_rows[0].get("split") if recent_rows else None,
                ...
                recent_activity.append(
                    {
                        "spec_id": spec.id,
                        "title": spec.title,
                        "message": f"Snapshot {row.get('label') or row.get('dataset_id')} frozen",
                        "status": "success",
                        "at": row.get("frozen_at"),
                    }
                )
        return {
            "summary": {
                ...
                "models_trained_from_snapshots": total_models,
            },
            "filters": {},
            "specs": cards,
            "recent_activity": recent_activity[:8],
            "lifecycle": ["Data", "Frozen Dataset", "Training", "Metrics"],
        }
    ...
    def training_overview(self) -> dict[str, Any]:
        ...
        for spec in specs:
            ...
            cards.append(
                self._overview_card(spec)
                | {
                    "status": "attention" if spec_failed else ("running" if spec_running else "healthy"),
                    "running_count": spec_running,
                    "completed_count": spec_completed,
                    "failed_count": spec_failed,
                    "latest_model": rows[0].get("model_id") if rows else None,
                    "last_frozen_dataset": rows[0].get("frozen_dataset_id") if rows else None,
                    "recent_runs": rows[:3],
                }
            )
            for row in rows[:2]:
                recent_activity.append(
                    {
                        "spec_id": spec.id,
                        "title": spec.title,
                        "message": f"Training run {row.get('run_id') or row.get('model_id')}",
        ...
        return {
            ...
            "filters": {},
            "specs": cards,
            "run_board": {"running": [], "recently_completed": [], "needs_attention": []},
            "recent_activity": recent_activity[:8],
        }
        ...
        for spec in specs:
            ...
            cards.append(
                ...
                | {
                    ...
                    "dimensions_supported": sorted({key for table in spec.metrics for key in table.entity_links}),
                }
            )
        return {
            "summary": {
                "metric_tables": sum(len(spec.metrics) for spec in specs),
                "metric_sources": len({table.metric_source for spec in specs for table in spec.metrics}),
                "models_covered": self._entity_count("model"),
                "frozen_datasets_linked": self._entity_count("frozen_dataset"),
            },
            "filters": {},
            "specs": cards,
            "architecture": ["Frozen Dataset", "Model", "Metric Table", "Dashboard Views"],
            "available_views": ["Model Metrics", "Tag Metrics", "Class Metrics", "Comparison Views", "Snapshot Views"],
        }
        ...
        for spec in specs:
            ...
            cards.append(
                ...
                | {
                    ...
                    "classes_tracked": len(class_keys),
                    "coverage": None,
                    "supported_filters": sorted({key for table in spec.class_metrics for key in table.entity_links}),
                }
            )
        return {
            "summary": {
                "class_metric_tables": sum(len(spec.class_metrics) for spec in specs),
                "total_tracked_classes": total_classes,
                "specifications_covered": len(specs),
                "filters_available": len({key for spec in specs for table in spec.class_metrics for key in table.entity_links}),
            },
            "filters": {},
            "specs": cards,
            "coverage_map": [],
            "explore_tiles": ["Browse by class", "Browse by tag", "Compare subsets", "Compare models"],
        }
    ...
    def frozen_rows(
        ...
        *,
        ...
        frozen = spec.frozen_dataset
        columns = [
            OpsColumn("dataset_id", "Dataset", frozen.id_column, "link", filterable=True, link_to="frozen_dataset"),
            OpsColumn("dataset", "Dataset", frozen.display_name_column or frozen.id_column, "text", filterable=True),
            OpsColumn("frozen_at", "Frozen at", frozen.created_at_column, "datetime"),
            *[
                OpsColumn(f"split_{split}", split.title(), column, "number", filterable=True)
                for split, column in frozen.split_columns.items()
            ],
        ]
    ...
    def training_rows(self, spec_id: str, **params: Any) -> dict[str, Any]:
        ...
        columns = [
            OpsColumn("model", "Model", training.model_id_column, "link", filterable=True, link_to="model"),
            OpsColumn("status", "Status", training.status_column, "status", filterable=True),
            *([OpsColumn("started", "Started", training.started_at_column, "datetime")] if training.started_at_column else []),
            *([OpsColumn("finished", "Finished", training.finished_at_column, "datetime")] if training.finished_at_column else []),
            *([OpsColumn("duration", "Duration", training.duration_seconds_column, "duration")] if training.duration_seconds_column else []),
            *training.extra_columns,
        ]
        ...
        for row in rows:
            ...
            row["run_id"] = run_id
            row["model_id"] = row.get(training.model_id_column)
            row["status"] = row.get(training.status_column)
            if training.started_at_column:
                row["started_at"] = row.get(training.started_at_column)
            if training.finished_at_column:
                row["finished_at"] = row.get(training.finished_at_column)
            if training.duration_seconds_column:
                row["duration_seconds"] = row.get(training.duration_seconds_column)
    ...
    def metric_table_rows(self, spec_id: str, table_id: str, *, class_metrics: bool = False, **params: Any) -> dict[str, Any]:
        spec = self.registry.get(spec_id)
        ...
        rows, total = self.query.rows(table_spec.table, allowed_columns=metric_table_columns(table_spec), **params)
        for row in rows:
    ...
    def _table_schema(self, table: Any) -> dict[str, Any]:
        return {
            ...
            "entity_links": dict(getattr(table, "entity_links", {}) or {}),
        }
