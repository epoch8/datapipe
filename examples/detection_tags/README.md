...
`DATAPIPE_TAGS_CACHE_DIR` (default `/tmp/datapipe-tags-cache`).
...
From RU (or anywhere COCO is blocked), fetch the pre-built 1000-image cache from the public bucket
instead:
```bash
set -a && source ../.env && set +a
