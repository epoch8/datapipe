...
input_dfs_tbl = Table(
    ...
    store=TableStoreFiledir(
        ...
        PandasParquetFile(pandas_column="data"),
        primary_schema=[
...
def batch_generate_df() -> Generator[pd.DataFrame, None, None]:
    ...
    for i in range(0, len(dates)):
        dfs.append(pd.DataFrame({"id": range(i + 2), "size": [8 + j for j in range(i + 2)]}))

    yield pd.DataFrame({"date_partition": [date.strftime("%Y-%m-%d") for date in dates], "data": dfs})
