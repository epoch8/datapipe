...
# Survivors of perceptual-hash dedup — this is what the SAM->CVAT tail consumes, so it stays identical
# to sam_cvat's `local_images`. video_id lingers in the PK for the per-video CVAT task split (and so
# dedup, grouped per video, can delete+reinsert a video's survivors); it is reduced away by sam_inference.
